...
const AnalystLeaderboard: React.FC<AnalystLeaderboardProps> = ({
  ...
  const renderCurrentUserCard = (agent) => {
    const rank = sortedAgents.findIndex(a => a.twitterHandle === agent.twitterHandle) + 1;
    ...
    const isCurrentUser = session?.user?.username && 
      cleanHandle.toLowerCase() === session.user.username.replace("@", "").toLowerCase();
    ...
    return (
      <div
        ...
        className={`impact-card list-item relative ${isCurrentUser 
          ...
          : "bg-gray-900/50 border-gray-800/50 hover:bg-cyan-950"} backdrop-blur-sm border rounded-lg overflow-hidden transition-all duration-200 hover:cursor-pointer`}
      ...
      >
          ...
          <div className="flex items-center flex-shrink-0 w-[240px] group-hover:translate-x-1 transition-transform duration-300">
            ...
            <div className="flex-grow overflow-hidden">
              <h4 className="text-sm font-semibold text-white truncate tracking-wide">
                {agent.name} {isCurrentUser && <span className="text-blue-300 font-medium">(You)</span>}
              </h4>
        ...
        <div className="lg:hidden">
          <MobileAnalystCard
            agent={isCurrentUser ? { ...agent, name: `${agent.name} (You)` } : agent}
            rank={rank}
  ...
  const currentUserAgentIndex = currentUserHandle
    ? sortedAgents.findIndex(agent =>
      agent.twitterHandle.toLowerCase().replace("@", "") === currentUserHandle.replace("@", ""))
    : -1;

  const currentUserAgent = currentUserAgentIndex >= 0 ? sortedAgents[currentUserAgentIndex] : null;
  const currentUserRank = currentUserAgentIndex >= 0 ? currentUserAgentIndex + 1 : null;
      ...
      {currentUserAgent && currentPage === 1 && (
        <div className="mb-6">
          <h3 className="ml-[10px] text-base sm:text-lg font-semibold text-gray-300 mb-3 sm:mb-4 flex items-center">
            <span className="mr-2"><FaUser></FaUser></span>
            Your Analytics
          </h3>
