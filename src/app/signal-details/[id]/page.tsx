...
export default function SignalDetailsPage() {
        ...
        return (
            <div className="min-h-screen flex items-center justify-center">
                <div className="bg-red-900 text-red-100 p-6 rounded-lg max-w-md text-center">
                    ...
                    <p>{error}</p>
                    <Link href="/profile" className="mt-4 inline-block px-4 py-2 bg-red-800 rounded-md hover:bg-red-700 transition-colors">
                        Return to Dashboard
                    </Link>
                </div>
            </div>
        ...
        return (
            <div className="min-h-screen flex items-center justify-center">
                <div className="bg-blue-900/50 text-yellow-100 p-6 rounded-lg max-w-md text-center">
                    <h2 className="text-xl font-bold mb-2 text-white">Signal Not Found</h2>
                    <p>The requested signal could not be found.</p>
                    <Link href="/profile" className="mt-4 inline-block px-4 py-2 bg-blue-800 rounded-md hover:bg-blue-700 transition-colors">
                        Return to Dashboard
                    </Link>
                </div>
            </div>
    ...
    const getSignalIcon = () => {
        ...
        if (signalType === "buy") return <ArrowUpCircle className="h-5 w-5 text-green-400" />;
        if (signalType === "sell") return <ArrowDownCircle className="h-5 w-5 text-red-400" />;
        if (signalType === "hold") return <PauseCircle className="h-5 w-5 text-blue-400" />;
    ...
    return (
        <div className="min-h-screen py-12 px-4 sm:px-6 lg:px-8">
            <div className="max-w-4xl mx-auto">
                <div className="mb-6">
                    <Link href="/profile" className="text-blue-400 hover:text-blue-300 flex items-center gap-2">
                ...
                <div className="bg-[#0c1420] rounded-xl shadow-lg overflow-hidden font-leagueSpartan">
                                    ...
                                    <div className="bg-[#111827]/50 rounded-lg p-4">
                                        ...
                                        <div className="space-y-4">
                                            <div>
                                                ...
                                                <p className="text-sm text-white font-medium">{signal.signal_data.token}</p>
                                            </div>
                                            <div>
                                                <p className="text-xs text-gray-400 mb-1">Date Generated</p>
                                                <p className="text-sm text-white">{formatDate(signal.generatedAt)}</p>
                                            </div>
                                            <div>
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                            ...
                                            {isExited ? (
                                                signal.signal_data.exitValue && (
                                                    <div>
                                                        ...
                                                        <p className="text-sm text-yellow-400 font-medium">${formatPrice(signal.signal_data.exitValue)}</p>
                                                    </div>
                                                )
                                            ) : (
                                                <div>
                                                    <p className="text-xs text-gray-400 mb-1">Stop Loss</p>
                                                    <p className="text-sm text-red-400 font-medium">${formatPrice(signal.signal_data.stopLoss)}</p>
                                                </div>
                                            )}
                                            ...
                                            {isExited && signal.signal_data.exitPnL && (
                                                <div>
                                                    ...
                                                    <p className={`text-sm font-medium ${signal.signal_data.exitPnL.includes('-') ? 'text-red-400' : 'text-green-400'}`}>
                                            ...
                                            {signal.signal_data.bestStrategy && (
                                                <div>
                                                    ...
                                                    <p className="text-sm text-blue-300">{signal.signal_data.bestStrategy}</p>
                                                </div>
                                            )}
                                                ...
                                                <div className="sm:col-span-2">
                                                    ...
                                                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                                                        {signal.signal_data.targets.map((target, idx) => (
                                                            <div key={idx} className="bg-[#1a2535] rounded px-3 py-2 flex items-center gap-2">
                                                                <Target className="h-4 w-4 text-green-400" />
                                                                <div>
                                                                    <p className="text-xs text-gray-400">TP{idx + 1}</p>
                                                                    <p className="text-sm text-green-400">${formatPrice(target)}</p>
                                                                </div>
                                                            </div>
                                                        ))}
                                                        <div className="bg-[#1a2535] rounded px-3 py-2 flex items-center gap-2">
                                                            ...
                                                            <div>
                                                                ...
                                                                <p className="text-sm text-red-400">${formatPrice(signal.signal_data.stopLoss)}</p>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                    ...
                    {/* Backtesting tab */}
                    {activeTab === "backtesting" && signal.backtestingDone && signal.backtestingData && (
                        <div className="p-6">
                            <div className="space-y-6">
                                <div className="bg-[#111827]/50 rounded-lg p-4">
                                    ...
                                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                                        <div>
                                            <p className="text-xs text-gray-400 mb-1">Signal Type</p>
                                            <p className="text-sm text-white">{signal.backtestingData["Signal Type"]}</p>
                                        </div>
                                        <div>
                                            <p className="text-xs text-gray-400 mb-1">Signal Date</p>
                                            <p className="text-sm text-white">{signal.backtestingData["Signal Generation Date"]}</p>
                                        </div>
                                        <div>
                                            <p className="text-xs text-gray-400 mb-1">Coin ID</p>
                                            <p className="text-sm text-white">{signal.backtestingData["Coin ID"]}</p>
                                        </div>
                                        <div>
                                            <p className="text-xs text-gray-400 mb-1">Entry Price</p>
                                            <p className="text-sm text-blue-400">${signal.backtestingData["Entry Price"] || 'N/A'}</p>
                                        </div>
                                        {signal.backtestingData["Exit Price"] && (
                                            <div>
                                                <p className="text-xs text-gray-400 mb-1">Exit Price</p>
                                                <p className="text-sm text-yellow-400">${formatPrice(signal.backtestingData["Exit Price"])}</p>
                                            </div>
                                        )}
                                        <div>
                                            <p className="text-xs text-gray-400 mb-1">Target 1</p>
                                            <p className="text-sm text-green-400">${signal.backtestingData["TP1"] || 'N/A'}</p>
                                        </div>
                                        {signal.backtestingData["TP2"] && (
                                            <div>
                                                <p className="text-xs text-gray-400 mb-1">Target 2</p>
                                                <p className="text-sm text-green-400">${signal.backtestingData["TP2"]}</p>
                                            </div>
                                        )}
                                        <div>
                                            <p className="text-xs text-gray-400 mb-1">Stop Loss</p>
                                            <p className="text-sm text-red-400">${signal.backtestingData["SL"] || 'N/A'}</p>
                                        </div>
                                        {signal.backtestingData["P&L"] && (
                                            <div>
                                                <p className="text-xs text-gray-400 mb-1">P&L</p>
                                                <p className={`text-sm ${signal.backtestingData["P&L"].includes('-') ? 'text-red-400' : 'text-green-400'}`}>
                                                    {signal.backtestingData["P&L"]}
                                                </p>
                                            </div>
                                        )}
                    ...
                    {activeTab === "tips" && signal.signal_data.tradeTip && (
                        <div className="p-6">
                            <div className="bg-[#111827]/50 rounded-lg p-4">
                                ...
                                <p className="text-sm text-gray-300 whitespace-pre-line">{signal.signal_data.tradeTip}</p>
                            </div>
                        </div>
                    )}
                </div>
